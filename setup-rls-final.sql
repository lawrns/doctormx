-- ========================================
-- Final RLS Policies Setup for Doctor.mx
-- ========================================
...
CREATE POLICY "referrals_insert_patient" ON referrals FOR INSERT WITH CHECK (auth.uid() = patient_id);

-- ========================================
-- APPOINTMENTS: Patients and providers
-- ========================================
ALTER TABLE appointments ENABLE ROW LEVEL SECURITY;

CREATE POLICY "appointments_select_patient" ON appointments FOR SELECT USING (auth.uid() = patient_id);
CREATE POLICY "appointments_select_provider" ON appointments FOR SELECT USING (auth.uid() IN (SELECT user_id FROM providers WHERE id = appointments.provider_id));
CREATE POLICY "appointments_insert_patient" ON appointments FOR INSERT WITH CHECK (auth.uid() = patient_id);
CREATE POLICY "appointments_update_both" ON appointments FOR UPDATE USING (
  auth.uid() = patient_id OR
  auth.uid() IN (SELECT user_id FROM providers WHERE id = appointments.provider_id)
);

...
CREATE POLICY IF NOT EXISTS "Allow authenticated users to read medical knowledge" ON medical_knowledge
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Allow service role to manage medical knowledge" ON medical_knowledge
...
ALTER TABLE health_scores ADD CONSTRAINT IF NOT EXISTS health_scores_level_check CHECK (level > 0);
...
-- Add RLS policies
CREATE POLICY IF NOT EXISTS "Users can view their own free questions" ON user_free_questions
    FOR SELECT USING (auth.uid() = user_id);
    
CREATE POLICY IF NOT EXISTS "Users can update their own free questions" ON user_free_questions
    FOR UPDATE USING (auth.uid() = user_id);
    
CREATE POLICY IF NOT EXISTS "Service role can manage all free questions" ON user_free_questions
    FOR ALL USING (auth.role() = 'service_role');

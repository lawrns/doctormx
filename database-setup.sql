...
('Dr. María García López', 'Medicina General', '55-1234-5678', 'maria.garcia@doctormx.com', 'Av. Reforma 123, Col. Centro, CDMX', 4.8, 15, ARRAY['español', 'inglés'], true),
('Dr. Carlos Mendez Rivera', 'Pediatría', '55-2345-6789', 'carlos.mendez@doctormx.com', 'Calle Madero 456, Col. Roma Norte, CDMX', 4.9, 12, ARRAY['español'], true),
('Dra. Ana Fernández Castro', 'Ginecología', '55-3456-7890', 'ana.fernandez@doctormx.com', 'Av. Insurgentes 789, Col. Condesa, CDMX', 4.7, 18, ARRAY['español', 'francés'], true),
('Dr. Roberto Silva Jiménez', 'Cardiología', '55-4567-8901', 'roberto.silva@doctormx.com', 'Paseo de la Reforma 321, Col. Polanco, CDMX', 4.9, 20, ARRAY['español', 'inglés'], true),
('Dra. Patricia Morales Vega', 'Dermatología', '55-5678-9012', 'patricia.morales@doctormx.com', 'Av. Universidad 654, Col. Del Valle, CDMX', 4.6, 10, ARRAY['español'], true)
ON CONFLICT DO NOTHING;
...
CREATE POLICY IF NOT EXISTS "Allow public read access to medical_knowledge" ON public.medical_knowledge FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Allow public read access to doctors" ON public.doctors FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Allow public read access to medications" ON public.medications FOR SELECT USING (true);

-- Allow public insert for appointments (patients can book appointments)
CREATE POLICY IF NOT EXISTS "Allow public insert to appointments" ON public.appointments FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Allow public read of own appointments" ON public.appointments FOR SELECT USING (true);

-- Prescriptions should be restricted (only doctors can create, patients can read their own)
CREATE POLICY IF NOT EXISTS "Allow authenticated users to read prescriptions" ON public.prescriptions FOR SELECT USING (auth.role() = 'authenticated');

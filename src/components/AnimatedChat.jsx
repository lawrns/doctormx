...
export default function AnimatedChat() {
  ...
  return (
    <div className="relative w-full max-w-sm mx-auto">
      {/* Phone frame */}
      <div className="relative rounded-[2.5rem] bg-gradient-to-br from-neutral-900 to-neutral-700 p-3 shadow-2xl">
        {/* Notch */}
        ...
        <div className="relative overflow-hidden rounded-[2rem] bg-white h-[600px]">
          {/* WhatsApp header */}
          <div className="bg-gradient-to-r from-accent-600 to-accent-500 px-4 py-3 flex items-center gap-3 shadow-md">
          ...
          <div className="p-4 space-y-3 h-[calc(100%-80px)] overflow-y-auto bg-gradient-to-b from-gray-50 to-white">
            {visibleMessages.map((msg) => (
              ...
              >
                {msg.type === 'referral' ? (
                  <div className="w-full bg-white border border-accent-200 rounded-xl p-3 shadow-sm">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 bg-gradient-to-br from-accent-500 to-accent-600 rounded-full flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
                        {msg.doctor.split(' ')[1]?.charAt(0) || 'D'}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="font-semibold text-neutral-900 text-sm">{msg.doctor}</p>
                        ...
                        <p className="text-xs text-accent-600 font-medium">{msg.specialty}</p>
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          <span className="text-[10px] text-neutral-500">{msg.distance}</span>
                        </div>
                      </div>
                      <button className="px-3 py-1.5 bg-gradient-to-r from-primary-600 to-primary-500 text-white text-xs font-semibold rounded-lg flex-shrink-0">
                        Ver perfil
                      </button>
                    </div>
                    <div className="text-[10px] text-neutral-500 mt-2">{msg.time}</div>
                  </div>
                ) : (
                  <div
                  ...
                  >
                    ...
                    <div className={`text-[10px] mt-1 ${msg.type === 'user' ? 'text-white/70' : 'text-neutral-500'}`}>
                      {msg.time}
                    </div>
                  </div>
                )}
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-neutral-200 px-4 py-2">
            <div className="flex items-center gap-2">
              <div className="flex-1 bg-gray-100 rounded-full px-4 py-2">
                <span className="text-sm text-neutral-500">Escribe un mensaje...</span>
              </div>
              <button className="w-10 h-10 rounded-full bg-accent-500 flex items-center justify-center">
      ...
      <div className="absolute -bottom-4 -left-4 bg-white rounded-xl shadow-lg p-3 border border-neutral-200 animate-fade-in-up" style={{animationDelay: '1s'}}>
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-xs font-semibold text-neutral-900">Verificado</div>
            <div className="text-[10px] text-neutral-600">NOM-004</div>
          </div>
        </div>
      </div>

      {/* Floating response time badge */}
      <div className="absolute -top-4 -right-4 bg-white rounded-xl shadow-lg p-3 border border-neutral-200 animate-fade-in-up" style={{animationDelay: '1.5s'}}>
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-xs font-semibold text-neutral-900">2 min</div>
            <div className="text-[10px] text-neutral-600">respuesta</div>
          </div>
        </div>
      </div>
    </div>

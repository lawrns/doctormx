...
function StatIcon({ type }: { type: string }) {
  switch (type) {
    ...
    case 'satisfaction':
      return (
        <svg className="w-8 h-8 text-[#0066CC]" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat, index) => (
            ...
            >
              <motion.div
                className="w-16 h-16 mx-auto mb-4 bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-100/50 group-hover:shadow-xl group-hover:shadow-blue-200/50 transition-shadow"
                whileHover={{ scale: 1.1, rotate: 5 }}
                transition={{ type: 'spring', stiffness: 300 }}
              >
                <StatIcon type={stat.iconType} />
              </motion.div>
              <div className="text-4xl sm:text-5xl font-bold bg-gradient-to-r from-[#0066CC] to-cyan-600 bg-clip-text text-transparent mb-2">

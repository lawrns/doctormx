 ...
 * for easy data transfer between services
 */
...
export async function exportUserDataForPortability(
  ...
  // Get medical records (SOAP consultations)
  const { data: consultations } = await supabase
    .from('soap_consultations')
  ...
  const formattedAppointments = (appointments || []).map((apt: AppointmentWithDoctor) => ({
    id: apt.id,
  ...
  const formattedConsultations = (consultations || []).map((consult: ConsultationRecord) => ({
    id: consult.id,
    type: 'SOAP',
  ...
  const formattedPrescriptions = (prescriptions || []).map((rx: PrescriptionRecord) => ({
    id: rx.id,
    date: rx.created_at,
  ...
  const formattedPayments = (payments || []).map((payment: PaymentRecord) => ({
    id: payment.id,
    date: payment.created_at,
    amount: (payment.amount_cents as number) / 100,
  ...
  return {
    ...
    exporting_service: {
      ...
      privacy_policy_url: `${process.env.NEXT_PUBLIC_APP_URL || 'https://doctormx.com'}/privacy`,
    },
    user: {
      id: profile.id,
      email: profile.email,
    ...
    metadata: {
      ...
      retention_policy_url: `${process.env.NEXT_PUBLIC_APP_URL || 'https://doctormx.com'}/privacy`,
    },
  }
}

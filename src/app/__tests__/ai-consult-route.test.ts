...
describe('/api/ai/consult POST', () => {
  ...
  it('runs SOAP consultation when enough information is available', async () => {
    vi.mocked(aiChatCompletion)
      .mockResolvedValueOnce({
        content: JSON.stringify({
          ...
          symptomsDescription: 'Dolor abdominal intenso con náusea',
          symptomDuration: '2 horas',
          symptomSeverity: 8,
          onsetType: 'sudden',
          associatedSymptoms: ['náusea'],
      ...
      .mockResolvedValueOnce({
        content: JSON.stringify({
          ...
          confidence: 0.9,
          redFlags: ['dolor abdominal intenso'],
          recommendedAction: 'book-appointment',
        }),
        usage: { inputTokens: 10, outputTokens: 10, totalTokens: 20 },
        costUSD: 0.001,
    ...
    vi.mocked(runSOAPConsultation).mockResolvedValue({
      ...
      assessment: {
        ...
        consensus: {
          ...
          urgencyLevel: 'urgent',
          combinedRedFlags: ['dolor abdominal intenso'],
      ...
      plan: {
        ...
        patientEducation: ['El dolor abdominal intenso debe valorarse pronto.'],
      },
      metadata: {
        totalTokens: 100,
        totalCostUSD: 0.1,
    ...
    const request = new Request('http://localhost/api/ai/consult', {
      ...
      body: JSON.stringify({
        messages: [
          { role: 'user', content: 'Tengo dolor abdominal' },
          { role: 'assistant', content: '¿Desde cuándo y qué tan intenso es?' },
          { role: 'user', content: 'Desde hace dos horas, es fuerte y tengo náusea' },

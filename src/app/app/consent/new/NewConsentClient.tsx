...
export function NewConsentClient({ userId, preselectedType }: NewConsentClientProps) {
  ...
  useEffect(() => {
    async function fetchConsentStatus() {
      try {
        const response = await apiRequest<ConsentStatusResponse>('/api/consent/status', {
          method: 'GET',
        })
        ...
        if (apiError.code === 'TIMEOUT') {
          logger.warn('Timeout fetching consent status')
        } else if (apiError.code === 'NETWORK_ERROR') {
          logger.warn('Network error fetching consent status')
        } else {
          logger.error('Error fetching consent status', { error: apiError.message })
        }
      } finally {
        setLoading(false)
      }
  ...
  const handleGrant = async () => {
    ...
    try {
      await apiRequest('/api/consent/grant', {
        method: 'POST',
        body: {
      ...
      if (apiError.code === 'TIMEOUT') {
        ...
        setError(apiError.message ?? 'Error al otorgar consentimiento')
      }
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      const statusResponse = await apiRequest<ConsentStatusResponse>('/api/consent/status', {
        method: 'GET',
      })
      ...
      await apiRequest('/api/consent/withdraw', {
        method: 'POST',
        body: {
      ...
      if (apiError.code === 'TIMEOUT') {
        ...
        setError(apiError.message ?? 'Error al retirar consentimiento')
      }

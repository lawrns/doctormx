...
import { ARCO_RIGHTS } from '../DataRightsClient'
...
export function NewArcoRequestClient({ userId, preselectedType }: NewArcoRequestClientProps) {
  ...
  useEffect(() => {
    if (preselectedType && (ARCO_TYPES as readonly string[]).includes(preselectedType)) {
      setSelectedType(preselectedType as typeof ARCO_TYPES[number])
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {ARCO_RIGHTS.map((right) => (
                  <button
                    key={right.type}
                    onClick={() => {
                      setSelectedType(right.type as typeof ARCO_TYPES[number])
                      setTitle(right.title)
                      setDescription(right.description)
                    }}

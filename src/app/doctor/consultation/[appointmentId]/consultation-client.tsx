...
interface ConsultationClientProps {
  ...
  intakeResponse: { responses_json: Record<string, unknown> } | null
}

export default function ConsultationClient({ appointment: initialAppointment, appointmentId, soapNote, intakeResponse }: ConsultationClientProps) {
  const [appointment, setAppointment] = useState(initialAppointment)
  const [roomData, setRoomData] = useState<{ roomUrl: string; token: string } | null>(null)
  const [patientIntake, setPatientIntake] = useState<Record<string, unknown> | null>(
    intakeResponse?.responses_json as Record<string, unknown> | null ?? null
  )
    ...
    async function loadSoapAndIntake() {
      try {
        ...
        if (soapRes.ok) {
          ...
          if (soapData.note) {
            // SOAP note handled by AutoSoapPanel via initialNote prop
          }
        }
      } catch {

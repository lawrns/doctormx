    ...
    return (
      <DoctorLayout profile={profile!} isPending={true} currentPath="/doctor/followups" pendingAppointments={0}>
        <div className="max-w-4xl">
          <div className="bg-warning-50 border-l-4 border-warning-400 p-6 rounded-r-lg mb-8" role="alert" aria-live="polite">
            <div className="flex items-start">
              <svg className="w-6 h-6 text-warning-600 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
              ...
              <div className="ml-4">
                <h3 className="text-lg font-semibold text-warning-900">Tu perfil está en revisión</h3>
    ...
    <DoctorLayout profile={profile!} isPending={false} currentPath="/doctor/followups" pendingAppointments={0}>
      <div className="max-w-6xl">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-3xl font-bold text-text-primary mb-2">Respuestas de Seguimiento</h2>
            <p className="text-text-muted">Revisa las respuestas de tus pacientes a los seguimientos automáticos</p>
          </div>
          <Link
            href="/doctor"
            className="text-sm text-primary hover:text-primary/90 font-medium transition-all duration-150 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded"
          >
            Volver al dashboard →
          </Link>
        </div>
        ...
        {responses.length === 0 ? (
          <div className="bg-card rounded-lg shadow-card border border-border p-12 text-center">
            ...
            <p className="text-text-secondary text-lg mb-2">Sin respuestas aún</p>
          ...
          <div className="bg-card rounded-lg shadow-card border border-border overflow-hidden">
            ...
            <div className="divide-y divide-border">
              {responses.map((response) => {
                ...
                return (
                  <div key={response.id} className="p-6 hover:bg-muted transition-colors duration-150">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <p className="text-text-secondary mb-2">"{response.response}"</p>
                      ...
                      <div className="ml-4 flex flex-col gap-2">
                        {response.action_taken === 'alert_doctor' && (
                          <Link
                            ...
                            className="inline-flex items-center gap-2 px-4 py-2 bg-destructive text-white text-sm font-medium rounded-lg hover:bg-destructive/90 transition-all duration-200 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                            aria-label={`Agendar seguimiento para ${response.patient_name}`}
                          >
                        ...
                        {response.action_taken === 'prescription_renewal' && (
                          <Link
                            ...
                            className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white text-sm font-medium rounded-lg hover:bg-primary/90 transition-all duration-200 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                            aria-label={`Renovar receta para ${response.patient_name}`}
                          >
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="mt-8 grid md:grid-cols-3 gap-6">
          <div className="bg-card rounded-lg shadow-card border border-border p-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h4 className="font-semibold text-text-primary">Mejoras Positivas</h4>
            </div>
            <p className="text-3xl font-bold text-text-primary mb-1">
              {responses.filter(r => r.action_taken === 'positive_outcome').length}
            </p>
            <p className="text-sm text-text-muted">pacientes mejorando</p>
          </div>
          ...
          <div className="bg-card rounded-lg shadow-card border border-border p-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h4 className="font-semibold text-text-primary">Requiere Atención</h4>
            </div>
            <p className="text-3xl font-bold text-text-primary mb-1">
              {responses.filter(r => r.action_taken === 'alert_doctor').length}
            </p>
            <p className="text-sm text-text-muted">pacientes con alertas</p>
          </div>
          ...
          <div className="bg-card rounded-lg shadow-card border border-border p-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h4 className="font-semibold text-text-primary">Renovaciones</h4>
            </div>
            <p className="text-3xl font-bold text-text-primary mb-1">
              {responses.filter(r => r.action_taken === 'prescription_renewal').length}
            </p>
            <p className="text-sm text-text-muted">renovaciones solicitadas</p>
          </div>
        </div>
      </div>
    </DoctorLayout>

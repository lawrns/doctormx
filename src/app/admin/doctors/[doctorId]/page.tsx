...
export default async function AdminDoctorDetailPage({
  ...
  return (
    <AdminShell profile={profile} currentPath="/admin/doctors">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-foreground">{doctorProfile?.full_name || 'Doctor'}</h1>
        <p className="text-muted-foreground">{doctorProfile?.email || 'Sin email'} · {doctorProfile?.phone || 'Sin telefono'}</p>
      </div>

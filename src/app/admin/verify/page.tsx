...
export default async function AdminVerificationPage() {
  ...
  return (
    <AdminShell profile={profile} currentPath="/admin/verify">
      <div className="mb-8">
      ...
      {!pendingDoctors || pendingDoctors.length === 0 ? (
        <EmptyState
          iconName="doctor"
          title="No hay doctores pendientes"
          description="No hay doctores pendientes de verificación."
        />
      ) : (
        <div className="space-y-4">
          {(pendingDoctors as PendingDoctor[]).map((doctor) => {
            const doctorProfile = getProfile(doctor)
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-xl font-semibold text-foreground">
                    Dr. {doctorProfile?.full_name || 'Sin nombre'}
                  </h3>
                  <p className="text-sm text-muted-foreground">{doctorProfile?.email || 'Sin email'}</p>
                  {doctorProfile?.phone && (

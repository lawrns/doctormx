...
// I18N-007: Generate locale-aware metadata for doctors directory
  ...
  return generateSEOMetadata(
    {
      path: '/doctores',
      title: locale === 'en'
        ? "Verified Doctors in Mexico | Doctor.mx Directory"
        : "Doctores Verificados en México | Directorio Doctor.mx",
      description: locale === 'en'
        ? "Find verified doctors in Mexico. Browse by specialty, compare prices, and book appointments online. All doctors are verified with professional license."
        : "Encuentra doctores verificados en México. Busca por especialidad, compara precios y agenda citas en línea. Todos los médicos están verificados con cédula profesional.",
      keywords: locale === 'en'
        ? "verified doctors Mexico, doctor directory, medical specialists, book doctor appointment, online medical consultation, certified doctors"
        : "doctores verificados México, directorio médico, especialistas, agendar cita médica, consulta médica en línea, doctores certificados",
    },
    locale
  )
}
      ...
      <main id="main-content" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Card className="p-6 mb-8 border-neutral-200">
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            {/* Search Input */}
            <form className="flex-1" action="/doctores" method="GET">
            ...
            <div className="flex gap-2">
              ...
              <Link
                href={`/doctores${buildQueryString({ sortOrder: params.sortOrder === 'asc' ? undefined : 'asc' })}`}
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            <Link href={`/doctores${buildQueryString({ appointmentType: undefined })}`}>
              <Badge
            ...
            <Link href={`/doctores${buildQueryString({ appointmentType: 'video' })}`}>
              <Badge
            ...
            <Link href={`/doctores${buildQueryString({ appointmentType: 'in_person' })}`}>
              <Badge
          ...
          <div className="flex flex-wrap gap-2">
            <Link href={`/doctores${buildQueryString({ specialty: undefined })}`}>
              <Badge
            ...
            {specialties.map((specialty: { id: string; slug: string; name: string }) => (
              <Link key={specialty.id} href={`/doctores${buildQueryString({ specialty: specialty.slug })}`}>
                <Badge

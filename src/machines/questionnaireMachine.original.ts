// Import with safety mechanisms to prevent race conditions
import { createMachine, assign, waitForXState } from '../shims/xstate-shim';
import { ensureDependenciesLoaded } from '../lib/dependency-loader';

// Ensure dependencies are loaded before this module runs
// This is a background promise - we'll still throw proper errors in the shim
// if dependencies aren't ready when the machine is created
ensureDependenciesLoaded().catch(err => {
  console.error('[QuestionnaireMachine] Failed to preload dependencies:', err);
});
import { z } from 'zod';
...
const initialContext: QuestionnaireContext = {
  bodyRegion: null,
  symptoms: [],
...
const isValidBodyRegion = (region: string): region is BodyRegion => {

...
// OPTIMIZED scraper with higher concurrency
const limit = pLimit(8); // Increased from 3 to 8 concurrent scrapes
const BASE_URL = 'https://www.doctoralia.com.mx';
const OUTPUT_DIR = './doctors_data';
const DELAY_BETWEEN_PAGES = 1000; // Reduced from 2000ms to 1000ms
...
async function downloadAndConvertImage(url, outputPath) {
  try {
    ...
    return null; // Silently fail on image errors
  }
}
...
async function scrapeDoctorProfile(page, profileUrl) {
  try {
    await page.goto(profileUrl, {
      ...
      timeout: 45000 // Reduced from 60000ms
    });
    ...
    return null; // Silently fail on profile errors
  }
}
...
async function scrapeAllResume() {
  console.log('🚀 OPTIMIZED Doctor Scraper - 8x Concurrency\n');
      ...
      for (const city of cities) {
        ...
        while (hasMore) {
          ...
          try {
            ...
            await listingPage.waitForTimeout(200); // Reduced from 300ms
        ...
        if (urlsToScrape.length > 0) {
          console.log(`  🔍 Scraping profiles (8x concurrency)...`);

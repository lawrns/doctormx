...
async function testScraper() {
  console.log('Testing updated scraper on sample profile...\n');
  ...
  const result = await scrapeDoctorProfile(page, testUrl);
  ...
  console.log(`About: ${result?.about ? '✓' : '✗'} (${result?.about?.length || 0} chars)`);
  console.log(`Addresses: ${result?.addresses?.length || 0} found`);
  console.log(`Services: ${result?.services?.length || 0} found`);
  console.log(`Insurance: ${result?.insurance?.length || 0} found`);
  console.log(`Education: ${result?.education?.length || 0} found`);
  console.log(`Languages: ${result?.languages?.length || 0} found`);
  console.log(`Years of Experience: ${result?.yearsOfExperience ? '✓' : '✗'}`);

  // Save to test file
  await fs.writeFile('test-result.json', JSON.stringify(result, null, 2));

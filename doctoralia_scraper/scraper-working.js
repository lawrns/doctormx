...
async function scrapeDoctorProfile(page, profileUrl) {
  try {
    ...
    const doctorData = await page.evaluate(() => {
      ...
      const nameEl = document.querySelector('[data-test-id="doctor-header-fullname"], h1');
      ...
      // Rating - parse from text
      const ratingText = document.body.textContent.match(/(\d+\.?\d*)\s*estrella/i);
      data.rating = ratingText ? parseFloat(ratingText[1]) : null;
      ...
      // Photo - actual doctor image
      const photoEl = document.querySelector('.doctor-avatar img, .unified-doctor-header__avatar img');
      data.photoUrl = photoEl?.getAttribute('src') || '';

      // About
      const aboutEl = document.querySelector('[data-test-id="about-description"], .about-description');
      ...
      svcs.forEach(s => {
        const name = s.querySelector('h4, h5')?.textContent?.trim();
        const price = s.querySelector('.price')?.textContent?.trim();
        if (name) data.services.push({ name, price: price || '' });
      });

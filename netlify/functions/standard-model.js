...
const { getCachedResponse, setCachedResponse } = require('./utils/redis-client');

// Log startup information for debugging
console.log('Standard model function loading with Redis caching...');
...
exports.handler = async function(event) {
  ...
  // Check Redis cache first (before expensive OpenAI call)
  try {
    ...
    if (cachedResponse) {
      console.log('Returning cached response - skipping OpenAI call');
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({
    ...
    console.warn('Cache check failed, proceeding to OpenAI:', cacheError.message);
  }
  ...
  try {
    ...
    // Cache the response for future requests (24 hour TTL)
    try {

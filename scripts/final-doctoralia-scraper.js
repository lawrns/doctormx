...
// Final scraper to extract comprehensive doctor data
async function scrapeFinalDoctorData() {
  console.log('🔍 Scraping final comprehensive doctor data from Doctoralia...');
  ...
  try {
    ...
    // Process each doctor
    for (let i = 0; i < Math.min(doctors.length, 15); i++) { // Limit to first 15 doctors
...
// Save doctors to database with correct schema
async function saveDoctorsToDatabase(doctors) {
  ...
  for (const doctor of doctors) {
    try {
      ...
      // Then, create or update doctor record with correct schema
...
async function main() {
  console.log('🇲🇽 Final Doctoralia Scraper');
  console.log('============================');
  console.log('🚀 Starting final comprehensive scraping process...');
  ...
  try {
    ...
    const doctors = await scrapeFinalDoctorData();
    
    // Step 3: Save to database
    const dbResult = await saveDoctorsToDatabase(doctors);
    
    console.log('🎉 Final comprehensive scraping completed successfully!');
...
export { scrapeFinalDoctorData, saveDoctorsToDatabase };

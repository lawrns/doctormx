...
-- Purpose: Create audit tables and triggers for security monitoring
-- Created: 2026-02-16
-- Security: Tracks all data modifications and security events
...
CREATE INDEX IF NOT EXISTS idx_security_audit_severity 
  ON security_audit(severity, created_at DESC) WHERE severity IN ('high', 'critical');
CREATE INDEX IF NOT EXISTS idx_security_audit_unacknowledged 
  ON security_audit(created_at) WHERE acknowledged_at IS NULL;

-- Feature flag audit indexes
CREATE INDEX IF NOT EXISTS idx_feature_flag_audit_flag_key 
  ON feature_flag_audit(flag_key, timestamp DESC);
CREATE INDEX IF NOT EXISTS idx_feature_flag_audit_user 
  ON feature_flag_audit(user_id, timestamp DESC);
...
CREATE INDEX IF NOT EXISTS idx_auth_audit_ip_address 
  ON auth_audit(ip_address) WHERE ip_address IS NOT NULL;

-- Data access audit indexes
CREATE INDEX IF NOT EXISTS idx_data_access_audit_user 
  ON data_access_audit(user_id, accessed_at DESC);
CREATE INDEX IF NOT EXISTS idx_data_access_audit_table 
  ON data_access_audit(table_name, accessed_at DESC);
...
-- Policies for security_audit
CREATE POLICY "Service role full access to security_audit"
  ON security_audit FOR ALL TO service_role USING (true) WITH CHECK (true);
...
CREATE POLICY "Service role full access to feature_flag_audit"
  ON feature_flag_audit FOR ALL TO service_role USING (true) WITH CHECK (true);

-- Policies for auth_audit
CREATE POLICY "Service role full access to auth_audit"
  ON auth_audit FOR ALL TO service_role USING (true) WITH CHECK (true);

CREATE POLICY "Users can view their own auth events"
  ON auth_audit FOR SELECT
  USING (user_id = auth.uid());

-- Policies for data_access_audit
CREATE POLICY "Service role full access to data_access_audit"
  ON data_access_audit FOR ALL TO service_role USING (true) WITH CHECK (true);

CREATE POLICY "Users can view their own data access"
...
-- Function to automatically log changes to tables
...
-- ============================================================================
-- SAMPLE AUDIT TRIGGERS (Apply to sensitive tables)
-- ============================================================================
...
-- Apply audit trigger to prescriptions table
...
WHERE aa.event_type = 'login_failed'
...
COMMENT ON TABLE audit_logs IS 'Comprehensive audit trail of all data modifications across the system';
COMMENT ON TABLE security_audit IS 'Security-related events including suspicious activity and policy violations';
COMMENT ON TABLE feature_flag_audit IS 'Audit trail for feature flag access and changes';
COMMENT ON TABLE auth_audit IS 'Authentication and authorization event logging';
COMMENT ON TABLE data_access_audit IS 'Tracking of data access patterns for compliance';

-- Migration: AI Referrals Tracking (Fixed)
-- Description: Add consultation_id tracking to appointments for AI→Doctor conversion analytics
-- Date: 2026-01-26
-- Fix: Use TEXT type for consultation_id to match soap_consultations.id type

-- Add consultation_id column to appointments table (TEXT not UUID)
-- This links appointments created from AI consultations back to the original consultation
ALTER TABLE appointments
ADD COLUMN IF NOT EXISTS consultation_id TEXT REFERENCES soap_consultations(id) ON DELETE SET NULL;

-- ai_referral column already exists, skip
-- CREATE INDEX already exists for ai_referral, skip
...
-- Drop existing materialized view if it exists (from failed migration)
DROP MATERIALIZED VIEW IF EXISTS ai_conversion_analytics CASCADE;
...
DROP TRIGGER IF EXISTS trigger_set_ai_referral_flag ON appointments;

-- Create trigger to auto-set ai_referral flag when consultation_id is set
CREATE OR REPLACE FUNCTION set_ai_referral_flag()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  IF NEW.consultation_id IS NOT NULL AND (NEW.ai_referral = FALSE OR NEW.ai_referral IS NULL) THEN
...
-- UPDATE appointments SET ai_referral = TRUE WHERE consultation_id IS NOT NULL AND (ai_referral = FALSE OR ai_referral IS NULL);
